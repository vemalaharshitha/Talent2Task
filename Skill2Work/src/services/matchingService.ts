...
export function calculateJobMatchWithSemantic(
  ...
  if (semanticEval && semanticEval.matches.length > 0) {
    matchedSkills = semanticEval.matchedSkills;
    missingSkills = semanticEval.missingSkills;
    isAiPowered = semanticEval.isAiPowered;
    skillMatches = semanticEval.matches;
    rawSemanticScore = Math.round(semanticEval.semanticSkillScore * 100);

    const calibrated = job.required_skills.length > 0
      ? calibrateSemanticSimilarity(semanticEval.semanticSkillScore)
      : 0.85;
    skillScore = Math.round(calibrated * 100);
  } else {
    ...
    const ratio = job.required_skills.length > 0
  ...
  const { demandScore, demandLevel, demandPercentage } = calculateLocalDemandScore(job.required_skills, demandStats);
  ...
  const weightedSum = (
    ...
    timeScore * weights.availability +
    experienceScore * weights.experience +
    demandScore * weights.localDemand +
    reliabilityScore * weights.reliability
  ) / (totalWeight || 1.0);

  const matchScore = Math.min(99, Math.max(15, Math.round(weightedSum)));
