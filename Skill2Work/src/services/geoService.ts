      ...
      (err) => {
        ...
        switch (err.code) {
          case err.PERMISSION_DENIED:
            msg = 'Location permission was denied. Please allow location access or select a Tamil Nadu city manually.';
            break;
 ...
 */
export function getCompassDirection(bearing: number): { code: string; label: string } {
  const directions = [
    { code: 'N', label: 'North' },
 ...
 */
export function getTravelEstimates(distanceKm: number): TravelEstimatesResult {
  // Road factor to account for actual road networks vs straight line (approx 1.3x)
  const roadDistanceKm = Math.max(0.2, distanceKm * 1.3);
...
export function generateRoutePoints(
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    const lat = lat1 + (lat2 - lat1) * fraction + lateralOffset;
    const lon = lon1 + (lon2 - lon1) * fraction - lateralOffset * 0.5;
    points.push([lat, lon]);
  }
  return points;
}
