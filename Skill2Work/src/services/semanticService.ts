...
class SemanticService {
  ...
  /**
   * Initializes the Sentence Transformer model (all-MiniLM-L6-v2)
   */
  public async initModel(): Promise<void> {
    ...
    this.initPromise = (async () => {
      try {
        ...
        console.warn('Sentence Transformer initialization notice (falling back to seed embeddings + baseline):', err);
        this.status = 'error';
        this.errorMessage = err?.message || 'Sentence Transformer unavailable';
        this.notify();
      } finally {
