...
export const ChatAssistantModal: React.FC<ChatAssistantModalProps> = ({
  ...
  const getInitialMessage = (targetLang: Language = activeLangFilter): ChatMessage => {
    ...
    if (targetLang === 'ta') {
      ...
      quickActions = isRecruiter
        ? [
            { type: 'NAVIGATE_TAB', label: '📍 அருகிலுள்ள பணியாளர்கள்', payload: { tab: 'explore', view: 'map', query: 'அருகிலுள்ள தொழிலாளர்களைக் காட்டு' } },
            { type: 'OPEN_POST_JOB', label: '📝 வேலை அறிவிப்பு உருவாக்கு' }
          ]
        : [
            { type: 'NAVIGATE_TAB', label: '📍 அருகிலுள்ள வேலைகள்', payload: { tab: 'explore', query: 'அருகிலுள்ள வேலைகளைக் காட்டு' } },
            { type: 'NAVIGATE_TAB', label: '📈 திறன் வழிகாட்டி', payload: { tab: 'my-gigs', query: 'நான் என்ன திறன்களைக் கற்க வேண்டும்?' } }
          ];
      ...
      quickActions = isRecruiter
        ...
        : [
            { type: 'NAVIGATE_TAB', label: '📍 पास के काम', payload: { tab: 'explore', query: 'पास के काम खोजें' } },
            { type: 'NAVIGATE_TAB', label: '📈 कौशल अनुशंसाएँ', payload: { tab: 'my-gigs', query: 'मुझे कौन से कौशल सीखने चाहिए?' } }
          ];
      ...
      quickActions = isRecruiter
        ? [
            { type: 'NAVIGATE_TAB', label: '📍 కార్మికులను చూడండి', payload: { tab: 'explore', view: 'map', query: 'నా దగ్గర కార్మికులను చూపించండి' } },
        ...
        : [
            ...
            { type: 'NAVIGATE_TAB', label: '📈 నైపుణ్య గైడ్', payload: { tab: 'my-gigs', query: 'నేను ఏ నైపుణ్యాలు నేర్చుకోవాలి?' } }
          ];
    } else {
  ...
  const executeAction = (action: ChatActionItem, isAuto = false) => {
    switch (action.type) {
          ...
          if (!isAuto) {
            setMessages(prev => [
              ...prev,
              {
                id: 'sys_' + Date.now(),
                sender: 'assistant',
                text: `🔍 **Job Details Loaded**: Opened details for **"${action.payload.job.title}"** (₹${action.payload.job.payout_amount || 750}).`,
                timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
              }
            ]);
          }
        ...
        if (action.payload?.jobId && onClaimJob) {
          ...
          setMessages(prev => [
            ...prev,
            {
              id: 'sys_' + Date.now(),
              sender: 'assistant',
              text: `⚡ **1-Click Application Sent!** Successfully claimed shift **"${action.payload.jobTitle || 'Gig'}"**.`,
              timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            }
          ]);
        }
        ...
        if (!isAuto) {
          setMessages(prev => [
            ...prev,
            {
              id: 'sys_' + Date.now(),
              sender: 'assistant',
              text: `📝 **Job Creator Opened**: Pre-filled specifications for **${action.payload?.category || 'new gig'}**.`,
              timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            }
          ]);
        }
      ...
      case 'NAVIGATE_TAB':
        ...
        if (!isAuto) {
          const rawLbl = (action.label || '').toLowerCase();
          if (rawLbl.includes('near me') || rawLbl.includes('அருகில்') || rawLbl.includes('find gigs') || rawLbl.includes('open gigs') || rawLbl.includes('வேலைகள்')) {
            handleSend(activeLangFilter === 'ta' ? 'அருகிலுள்ள வேலைகளைக் காட்டு' : 'Find gigs near me');
          } else if (rawLbl.includes('skills') || rawLbl.includes('திறன்') || rawLbl.includes('demand') || rawLbl.includes('வழிகாட்டி')) {
            handleSend(activeLangFilter === 'ta' ? 'நான் என்ன திறன்களைக் கற்க வேண்டும்?' : 'What skills should I learn to earn more?');
          } else if (rawLbl.includes('candidate') || rawLbl.includes('பணியாளர்கள்') || rawLbl.includes('talent')) {
            handleSend(activeLangFilter === 'ta' ? 'அருகிலுள்ள தொழிலாளர்களைக் காட்டு' : 'Find verified candidates near me');
          }
        }
        break;
      case 'SET_RADIUS':
        if (action.payload?.radiusKm && onSetRadius) {
          ...
          if (!isAuto) {
            ...
            handleSend(`Show jobs within ${action.payload.radiusKm} km`);
          }
        }
        break;
      case 'SET_LOCATION':
        if (action.payload?.city && onSelectCity) {
          ...
          if (!isAuto) {
            handleSend(`Find jobs in ${action.payload.city}`);
          }
        }
        break;
      case 'TRIGGER_GPS':
      ...
      case 'FILTER_JOBS':
        if (onFilterJobs) {
          ...
          if (!isAuto) {
            onNavigateTab('explore');
            const q = action.payload?.searchQuery || action.payload?.category || 'Show active jobs';
            handleSend(q);
          }
        }
        break;
        ...
        if (action.payload?.skill && onAddSkillToProfile) {
          ...
          setMessages(prev => [
            ...prev,
            {
              id: 'sys_' + Date.now(),
              sender: 'assistant',
              text: `✅ **Skill Added**: **${action.payload.skill}** has been added to your worker profile!`,
              timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            }
          ]);
        }
      ...
      case 'OPEN_COMMUNITY_DEMAND':
        if (onOpenCommunityDemand) {
          ...
          onNavigateTab('my-gigs');
        }
        if (!isAuto) {
          handleSend(activeLangFilter === 'ta' ? 'மாநில தேவை வரைபடம்' : 'What skills are in demand in my city?');
        }
        break;
      case 'OPEN_PROFILE':
        if (onOpenProfile) {
      ...
      case 'CONFIRM_ACTION':
        ...
        break;
      case 'START_VOICE_INTERVIEW':
        if (action.payload?.autoStartMic) {
          handleToggleVoiceInput();
        } else {
          handleSend(activeLangFilter === 'ta' ? 'குரல் மூலம் சுயவிவரம் உருவாக்கு' : 'Start voice profile interview');
        }
        break;
      case 'START_SAFEGIG':
        if (onRequestLiveGps) onRequestLiveGps();
        setMessages(prev => [
          ...prev,
          {
            id: 'sys_' + Date.now(),
            sender: 'assistant',
            text: '🛡️ **SafeGig Sentinel Active**: Live GPS tracking enabled. Emergency SOS (112) is armed and your active work radius is monitored.',
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
          }
        ]);
        break;
        ...
        if (action.payload?.text) {
          ...
          setMessages(prev => [
            ...prev,
            {
              id: 'sys_' + Date.now(),
              sender: 'assistant',
              text: '📋 **Copied to Clipboard!** You can now paste and send this counter-offer directly to the recruiter or client.',
              timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            }
          ]);
        }
      ...
      case 'SIMULATE_SMS':
        setMessages(prev => [
          ...prev,
          {
            id: 'sys_' + Date.now(),
            sender: 'assistant',
            text: `📲 **Instant Alert Subscribed!** You will receive real-time SMS & WhatsApp alerts for new **${action.payload?.trade || 'matched'}** shifts in your area.`,
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
          }
        ]);
        break;
      case 'BOOK_WORKER':
        if (action.payload?.workerName) {
          setMessages(prev => [
            ...prev,
            {
              id: 'sys_' + Date.now(),
              sender: 'assistant',
              text: `✅ **Direct Booking Dispatched!** Booking request sent to **${action.payload.workerName}** (${action.payload.skill || 'Worker'}). You will receive confirmation shortly.`,
              timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            }
          ]);
        }
        break;
      case 'UPDATE_PROFILE':
        if (action.payload?.trade && onAddSkillToProfile) {
          onAddSkillToProfile(action.payload.trade);
        }
        if (onOpenProfile) {
          onOpenProfile();
        }
        setMessages(prev => [
          ...prev,
          {
            id: 'sys_' + Date.now(),
            sender: 'assistant',
            text: `✅ **Worker Profile Updated!** Registered Trade: **${action.payload?.trade || 'Skill'}** | Location: **${action.payload?.location || 'Tamil Nadu'}**`,
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
          }
        ]);
        break;
    ...
    if (isRecruiter) {
      if (activeLangFilter === 'ta') {
        return [
          ...
          { title: 'உடனடி பதிவு', icon: <Sparkles className="w-3.5 h-3.5 text-purple-600" />, prompt: 'நாளை காலைக்கு பிளம்பர் புக் செய்ய வேண்டும்' },
          { title: 'பாதுகாப்பு விபரங்கள்', icon: <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />, prompt: 'SafeGig பாதுகாப்பு வசதிகள் என்ன?' }
        ];
      }
      ...
      return [
        ...
        { title: 'Instant Book', icon: <Sparkles className="w-3.5 h-3.5 text-purple-600" />, prompt: 'I need to book a plumber for pipe leak tomorrow morning' },
        { title: 'SafeGig Sentinel', icon: <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />, prompt: 'What is SafeGig safety features?' }
      ];
    } else {
      if (activeLangFilter === 'ta') {
        return [
          { title: '🌅 இன்றைய நிலவரம்', icon: <Sparkles className="w-3.5 h-3.5 text-amber-500" />, prompt: 'இன்றைய வேலை நிலவரம் என்ன' },
          { title: '🎙️ குரல் சுயவிவரம்', icon: <Mic className="w-3.5 h-3.5 text-sky-600" />, prompt: 'குரல் மூலம் சுயவிவரம் உருவாக்கு' },
          { title: '🤝 சம்பள சரிபார்ப்பு', icon: <TrendingUp className="w-3.5 h-3.5 text-emerald-600" />, prompt: 'சம்பள பேச்சுவார்த்தை செய்ய உதவு' },
          { title: '🛵 நிகர லாபம்', icon: <MapPin className="w-3.5 h-3.5 text-indigo-600" />, prompt: 'இந்த வேலைக்கான பயண செலவு மற்றும் நிகர லாபம் என்ன?' },
          { title: 'திறன் வேலைகள்', icon: <Briefcase className="w-3.5 h-3.5 text-sky-600" />, prompt: 'என் திறன்களுக்கு ஏற்ற வேலைகள்' },
          { title: '🛡️ சேஃப்கிக் பாதுகாப்பு', icon: <ShieldCheck className="w-3.5 h-3.5 text-rose-600" />, prompt: 'What is SafeGig safety features?' }
        ];
      }
      if (activeLangFilter === 'hi') {
        return [
          { title: '🌅 आज का पल्स', icon: <Sparkles className="w-3.5 h-3.5 text-amber-500" />, prompt: "Show me today's gig pulse" },
          { title: '🎙️ ध्वनि प्रोफाइल', icon: <Mic className="w-3.5 h-3.5 text-sky-600" />, prompt: 'Start voice interview to build my profile' },
          { title: '🤝 वेतन बातचीत', icon: <TrendingUp className="w-3.5 h-3.5 text-emerald-600" />, prompt: 'Employer offered me 600 for plumbing in Velachery, is this fair?' },
          { title: 'कौशल कार्य', icon: <Briefcase className="w-3.5 h-3.5 text-sky-600" />, prompt: 'मेरे कौशल से मेल खाते काम दिखाएं' }
        ];
      }
      if (activeLangFilter === 'te') {
        return [
          { title: '🌅 నేటి గిగ్ పల్స్', icon: <Sparkles className="w-3.5 h-3.5 text-amber-500" />, prompt: "Show me today's gig pulse" },
          { title: '🎙️ వాయిస్ ప్రొఫైల్', icon: <Mic className="w-3.5 h-3.5 text-sky-600" />, prompt: 'Start voice interview to build my profile' },
          { title: 'నైపుణ్య పనులు', icon: <Briefcase className="w-3.5 h-3.5 text-sky-600" />, prompt: 'నా నైపుణ్యాలకు సరిపోయే పనులు చూపించు' },
          { title: 'సమీపంలో', icon: <MapPin className="w-3.5 h-3.5 text-emerald-600" />, prompt: '5 కి.மீ పరిధిలో పనులు' }
        ];
      }
      return [
        { title: '🌅 Gig Pulse', icon: <Sparkles className="w-3.5 h-3.5 text-amber-500" />, prompt: "Show me today's gig pulse" },
        { title: '🎙️ Voice Profile', icon: <Mic className="w-3.5 h-3.5 text-sky-600" />, prompt: 'Start voice interview to build my profile' },
        { title: '🤝 Wage Check', icon: <TrendingUp className="w-3.5 h-3.5 text-emerald-600" />, prompt: 'Employer offered me 600 for plumbing in Velachery, is this fair?' },
        { title: '🛵 Net Profit', icon: <MapPin className="w-3.5 h-3.5 text-indigo-600" />, prompt: 'What is my net profit and travel cost for this job?' },
        { title: 'Smart Match', icon: <Briefcase className="w-3.5 h-3.5 text-sky-600" />, prompt: 'Show jobs matching my skills' },
        { title: '🛡️ SafeGig Guard', icon: <ShieldCheck className="w-3.5 h-3.5 text-rose-600" />, prompt: 'What is SafeGig safety features?' }
      ];
    }
  ...
  useEffect(() => {
    if (externalPrompt && externalPrompt.trim() && externalPrompt !== lastExternalPromptRef.current) {
      ...
      handleSend(externalPrompt.trim());
    }
  }, [externalPrompt]);
                            ...
                            {msg.actions.map((act, i) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`px-3 py-1.5 border rounded-lg text-[11px] transition-all flex items-center gap-1.5 shadow-xs hover:shadow-md active:scale-95 cursor-pointer ${btnClasses}`}
                                >
                                  <span>{act.label}</span>
                                  <ArrowRight className={`w-3.5 h-3.5 ${isDanger ? 'text-red-700' : isSuccess ? 'text-emerald-700' : 'text-sky-700'}`} />
                                </button>
                              );
                            })}
                          </div>
