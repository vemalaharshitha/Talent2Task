...
const FitRouteBounds: React.FC<FitRouteBoundsProps> = ({ origin, destination }) => {
  ...
  useEffect(() => {
    try {
      if (!origin || !destination || isNaN(origin[0]) || isNaN(destination[0])) return;
      if (Math.abs(origin[0] - destination[0]) < 0.0001 && Math.abs(origin[1] - destination[1]) < 0.0001) {
...
export const GigDirectionsModal: React.FC<GigDirectionsModalProps> = ({
  ...
  const originCoords = useMemo<[number, number]>(() => {
    if (liveLocation && !isNaN(Number(liveLocation.lat)) && !isNaN(Number(liveLocation.lng))) {
      return [Number(liveLocation.lat), Number(liveLocation.lng)];
    }
    if (currentUser?.latitude && currentUser?.longitude && !isNaN(Number(currentUser.latitude)) && !isNaN(Number(currentUser.longitude))) {
      return [Number(currentUser.latitude), Number(currentUser.longitude)];
    }
  ...
  const destCoords = useMemo<[number, number]>(() => {
    if (recruiterObj?.latitude && recruiterObj?.longitude && !isNaN(Number(recruiterObj.latitude)) && !isNaN(Number(recruiterObj.longitude))) {
      return [Number(recruiterObj.latitude), Number(recruiterObj.longitude)];
    }
  ...
  const compassDir = getCompassDirection(bearingDegrees) || { code: 'N', label: 'North' };
  ...
  const offlineGeoNavUrl = buildNavigationUrl(
    originCoords[0],
    originCoords[1],
    destCoords[0],
    destCoords[1],
  ...
  const handleCopyCoords = () => {
    navigator.clipboard.writeText(`${destCoords[0].toFixed(5)}, ${destCoords[1].toFixed(5)}`);
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-md animate-fadeIn overflow-y-auto">
      <div 
      ...
      >
        ...
        <div className="p-4 sm:p-5 overflow-y-auto space-y-4 text-sm bg-slate-50/50">
          ...
          <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3">
            ...
            <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between text-xs text-slate-600">
              <div className="flex items-center gap-1.5">
                ...
                <span>Head <strong>{compassDir?.label || 'Direct'} ({compassDir?.code || 'N'})</strong> towards {localizedLandmark || recruiterAddress}</span>
              </div>
              <span className="text-[11px] font-mono text-slate-600">
                {(bearingDegrees || 0).toFixed(0)}° Bearing
              </span>
            </div>
          </div>
          ...
          <div className="h-56 sm:h-64 rounded-2xl overflow-hidden border border-slate-200 shadow-inner relative">
            <MapContainer
              key={`route-map-${originCoords[0]}-${originCoords[1]}-${destCoords[0]}-${destCoords[1]}`}
              center={originCoords}
              zoom={13}
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="py-2 px-3 rounded-xl font-bold text-xs bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 flex items-center justify-center gap-1.5 transition-colors shadow-2xs"
              >
              ...
              <a
                href={offlineGeoNavUrl}
                className="col-span-2 sm:col-span-1 py-2 px-3 rounded-xl font-bold text-xs bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center justify-center gap-1.5 transition-colors shadow-2xs"
                title="Direct geo URI for offline apps (Organic Maps, OsmAnd, Maps.me, Google Maps Offline)"
              >
                <LocateFixed className="w-3.5 h-3.5 text-emerald-600" />
                <span>Offline GPS (geo:)</span>
              </a>
            </div>

            <div className="flex items-center justify-between text-xs text-slate-500 bg-slate-100/70 px-3 py-1.5 rounded-xl border border-slate-200">
              <span className="font-mono text-[11px] truncate">
                📍 {destCoords[0].toFixed(5)}° N, {destCoords[1].toFixed(5)}° E
              </span>
              <button
                type="button"
                onClick={handleCopyCoords}
                className="font-bold text-[11px] text-sky-600 hover:text-sky-700 flex items-center gap-1 shrink-0 ml-2 cursor-pointer"
              >
                {copiedCoords ? (
                  <>
