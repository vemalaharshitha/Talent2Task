...
export const SeekerProfileModal: React.FC<SeekerProfileModalProps> = ({
  ...
  const [jobRole, setJobRole] = useState(user.job_role || user.skills?.[0] || 'General Worker');
  ...
  useEffect(() => {
    if (isOpen && user) {
      setName(user.name);
      setJobRole(user.job_role || user.skills?.[0] || 'General Worker');
      setAge(user.age);
      ...
      setFreeTimeSlots(user.free_time_slots || []);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (emergencyContactPhone.trim()) {
      ...
      if (cleanPhone.length < 10) {
        setEmergencyPhoneError('Please enter a valid 10-digit emergency contact phone number');
        return;
      }
    }
    setEmergencyPhoneError('');
    ...
    const updated: User = {
      ...user,
      name,
      job_role: jobRole.trim() || skills[0] || 'General Worker',
      age: Number(age),
      ...
      >
        ...
        <form onSubmit={handleSave} className="p-5 sm:p-6 overflow-y-auto space-y-5 text-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <div className="sm:col-span-2 space-y-1">
              <label className="text-xs font-semibold text-slate-700 flex items-center gap-1">
                ...
                <span>Job Role / Primary Designation</span>
              </label>
              <input
                type="text"
                required
                ...
                placeholder="e.g. Electrician, Delivery Driver, Plumber..."
                className="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-900 focus:outline-none focus:border-sky-500 focus:bg-white focus:ring-1 focus:ring-sky-500"
              />
            </div>
              ...
              {skillsList.map((skill) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${isSelected
              ...
              {TIME_SLOT_OPTIONS.map((slot) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-2 rounded-xl text-xs font-semibold transition-all border flex items-center justify-between ${isSelected
          ...
          <div className="space-y-2.5 p-4 rounded-2xl bg-slate-50 border border-slate-200">
              ...
              {cityLandmarks.map((loc) => {
                ...
                return (
                  <button
                    ...
                    className={`p-2 rounded-lg text-[11px] font-medium text-left transition-all border ${isSelected
            ...
            {/* SafeGig Emergency Contact (Optional) */}
            <div className="space-y-2.5 pt-3 border-t border-slate-200">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-400 font-normal">Used during SOS</span>
              </div>
              <p className="text-[11px] text-slate-500 leading-relaxed">
                Add an emergency contact for SafeGig. During an emergency, this enables one-touch calling and SMS alert preparation. This is kept private and never displayed publicly.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="e.g. Ramesh (Family / Friend)"
                    className="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-900 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-all placeholder:text-slate-400"
                  />
                </div>

                <div className="space-y-1">
                  <label className="text-[11px] font-semibold text-slate-600">Emergency Phone Number</label>
                  <input
                    type="tel"
                    ...
                    className={`w-full px-3.5 py-2 bg-white border rounded-xl text-xs sm:text-sm text-slate-900 focus:outline-none transition-all placeholder:text-slate-400 ${
                      emergencyPhoneError 
                  ...
                  />
                  {emergencyPhoneError && (
                    <p className="text-[10px] text-rose-600 font-medium">{emergencyPhoneError}</p>
                  )}
                </div>
              </div>
            </div>
