...
export interface User {
  ...
  created_at?: string;
  job_role?: string; // Specific primary job role / designation
  emergency_contact_name?: string; // SafeGig emergency contact person
  emergency_contact_phone?: string; // SafeGig emergency contact phone number
}

// ==========================================
// SafeGig: Worker Safety & Emergency Check-In Types
// ==========================================
export type SafeGigStatus = 'ACTIVE' | 'COMPLETED' | 'SOS_TRIGGERED';

export interface SafeGigSession {
  id: string; // sg_session_XXXXXXXX
  worker_id: string;
  job_id: string;
  job_title: string;
  job_location: string;
  started_at: string; // ISO 8601 string
  checked_out_at?: string; // ISO 8601 string
  duration_seconds: number;
  status: SafeGigStatus;
