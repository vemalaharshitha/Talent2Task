import { INIT_DATABASE_SQL, CREATE_TRANSACTIONS_TABLE_SQL, CREATE_RECOMMENDATION_OUTCOMES_TABLE_SQL, CREATE_SAFEGIG_SESSIONS_TABLE_SQL } from './schema';
...
const FALLBACK_SAFEGIG_KEY = 'talent2task_safegig_fallback_v1';
...
class SQLiteManager {
  ...
  private memorySafeGigSessions: SafeGigSession[] = [];
  ...
  private initSyncListener() {
    syncService.subscribe((msg: SyncMessage) => {
      switch (msg.type) {
        ...
        case 'SAFEGIG_UPDATE':
          if (msg.data) {
            this.applyRemoteSafeGigUpdate(msg.data);
          }
          break;
        default:
          break;
  ...
  private initFallbackData() {
    try {
      ...
      const cachedSafeGig = localStorage.getItem(FALLBACK_SAFEGIG_KEY);

      if (cachedSafeGig) {
        try {
          this.memorySafeGigSessions = JSON.parse(cachedSafeGig);
        } catch {}
      }
      ...
      if (cachedUsers && cachedJobs) {
        ...
        this.memoryUsers = INITIAL_USERS.map(u => ({
          ...
          longitude: u.longitude,
          experience: (u as any).experience || 0,
          job_role: (u as any).job_role || (u.skills ? (JSON.parse(u.skills)[0] || 'General Worker') : 'General Worker'),
          city: (u as any).city || 'Chennai',
          address: (u as any).address || '',
          landmark: (u as any).landmark || ''
        }));
  ...
  private saveFallbackData() {
    try {
      ...
      localStorage.setItem(FALLBACK_SAFEGIG_KEY, JSON.stringify(this.memorySafeGigSessions));
    } catch (e) {
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      let initSqlJs = (window as any).initSqlJs;
      if (typeof initSqlJs !== 'function') {
        // Wait briefly if sql-wasm.js script tag is still resolving
        for (let i = 0; i < 15 && typeof (window as any).initSqlJs !== 'function'; i++) {
      ...
      console.warn('SQLite WASM initialization notice (using local SQL engine):', err);
    } finally {
      this.isReady = true;
  ...
  private migrateTables() {
    ...
    try {
      this.db.run("ALTER TABLE users ADD COLUMN job_role TEXT;");
    } catch {}
    try {
    ...
    try {
      this.db.run("ALTER TABLE users ADD COLUMN emergency_contact_name TEXT;");
    } catch {}
    try {
      this.db.run("ALTER TABLE users ADD COLUMN emergency_contact_phone TEXT;");
    } catch {}
    try {
      this.db.run(CREATE_SAFEGIG_SESSIONS_TABLE_SQL);
    } catch {}
  }
  ...
  private initTablesAndSeed() {
    ...
    const userStmt = this.db.prepare(
      `INSERT OR IGNORE INTO users (id, role, name, age, phone, skills, free_time_slots, preferred_language, latitude, longitude, experience, job_role, city, address, landmark, door_no, street_name, district) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    );
    INITIAL_USERS.forEach(u => {
      userStmt.run([
        ...
        (u as any).job_role || (u.skills ? (JSON.parse(u.skills)[0] || 'General Worker') : 'General Worker'),
  ...
  private syncMemoryFromDb() {
    if (this.db && this.isWasm) {
      try {
        ...
        const safegig = this.getSafeGigSessionsFromDb();
        ...
        if (safegig.length > 0) this.memorySafeGigSessions = safegig;
      } catch (e) {
  ...
  private getUsersFromDb(): User[] {
    ...
    return values.map((row: any[]) => {
      ...
      return {
        ...
        job_role: obj.job_role || (obj.skills ? (JSON.parse(obj.skills)[0] || 'General Worker') : 'General Worker'),
        ...
        emergency_contact_name: obj.emergency_contact_name || undefined,
    ...
    if (this.db && this.isWasm) {
      try {
        const stmt = this.db.prepare(`
          INSERT INTO users (id, role, name, age, phone, skills, free_time_slots, preferred_language, latitude, longitude, experience, job_role, city, address, landmark, door_no, street_name, district, emergency_contact_name, emergency_contact_phone)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        stmt.run([
          ...
          user.longitude,
          user.experience ?? 0,
          user.job_role || (user.skills && user.skills[0]) || 'General Worker',
          user.city || 'Chennai',
          user.address || '',
          user.landmark || '',
          user.door_no || '',
  ...
  // ====================================================
  // SafeGig: Worker Safety & Emergency Check-In
  // ====================================================

  private getSafeGigSessionsFromDb(): SafeGigSession[] {
    if (!this.db) return [];
    try {
      const results = this.db.exec('SELECT * FROM safegig_sessions ORDER BY started_at DESC, id DESC');
      if (!results || results.length === 0) return [];
      const { columns, values } = results[0];
      return values.map((row: any[]) => {
        const obj: any = {};
        ...
        return {
          ...
          checked_out_at: obj.checked_out_at || undefined,
          duration_seconds: Number(obj.duration_seconds || 0),
          status: obj.status as SafeGigStatus,
          sos_activated: Boolean(obj.sos_activated),
          sos_timestamp: obj.sos_timestamp || undefined,
          location_lat: obj.location_lat !== null && obj.location_lat !== undefined ? Number(obj.location_lat) : undefined,
          location_lng: obj.location_lng !== null && obj.location_lng !== undefined ? Number(obj.location_lng) : undefined,
          location_address: obj.location_address || undefined,
          location_available: Boolean(obj.location_available),
          safety_note: obj.safety_note || undefined,
          sync_status: (obj.sync_status || 'PENDING') as 'SYNCED' | 'PENDING',
          created_at: obj.created_at || obj.started_at
        };
      });
    } catch (e) {
      console.warn('getSafeGigSessionsFromDb error:', e);
      return [];
    }
  }

  public getSafeGigSessions(): SafeGigSession[] {
    if (this.db && this.isWasm) {
      try {
        const dbSessions = this.getSafeGigSessionsFromDb();
        if (dbSessions.length > 0) {
          this.memorySafeGigSessions = dbSessions;
        }
      } catch (e) {
        console.warn('getSafeGigSessionsFromDb error:', e);
      }
    }
    return [...this.memorySafeGigSessions];
  }

  public getActiveSafeGigSession(workerId: string): SafeGigSession | null {
    const sessions = this.getSafeGigSessions();
    return sessions.find(s => s.worker_id === workerId && s.status === 'ACTIVE') || null;
  }

  public getSafeGigSessionsByWorker(workerId: string): SafeGigSession[] {
    const sessions = this.getSafeGigSessions();
    return sessions.filter(s => s.worker_id === workerId);
  }

  public getSafeGigSessionForJob(workerId: string, jobId: string): SafeGigSession | null {
    const sessions = this.getSafeGigSessions();
    return sessions.find(s => s.worker_id === workerId && s.job_id === jobId && s.status === 'ACTIVE') || null;
  }

  public startSafeGigSession(
    workerId: string, 
    jobId: string, 
    jobTitle: string, 
    jobLocation: string,
    ...
    // 1. Prevent duplicate active sessions for worker
    const existing = this.getActiveSafeGigSession(workerId);
    if (existing) {
      return existing;
    }

    const nowIso = new Date().toISOString();
    const newSession: SafeGigSession = {
      id: `sg_sess_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`,
      worker_id: workerId,
      job_id: jobId,
      job_title: jobTitle,
      ...
      sync_status: isRemote ? 'SYNCED' : 'PENDING',
      created_at: nowIso
    };

    this.memorySafeGigSessions.unshift(newSession);

    if (this.db && this.isWasm) {
      try {
        const stmt = this.db.prepare(`
          INSERT INTO safegig_sessions (id, worker_id, job_id, job_title, job_location, started_at, duration_seconds, status, sos_activated, location_available, sync_status, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
        stmt.run([
          newSession.id,
          newSession.worker_id,
          newSession.job_id,
          newSession.job_title,
          newSession.job_location,
          newSession.started_at,
          0,
          'ACTIVE',
          0,
          0,
          newSession.sync_status,
          newSession.created_at
        ]);
        stmt.free();
      } catch (e) {
        console.warn('SQLite startSafeGigSession error:', e);
      }
    }
    ...
    if (!isRemote) {
      if (!offlineQueueService.isOnline()) {
        offlineQueueService.enqueueAction('START_SAFEGIG', newSession);
      } else {
        syncService.broadcastSafeGigUpdate({ action: 'START', session: newSession });
      }
    }
  ...
  public completeSafeGigSession(
    sessionId: string, 
    safetyNote?: string, 
    isRemote: boolean = false
  ): SafeGigSession | null {
    const session = this.memorySafeGigSessions.find(s => s.id === sessionId);
    if (!session) return null;

    const checkedOutAt = new Date().toISOString();
    const startMs = new Date(session.started_at).getTime();
    const durationSeconds = Math.max(0, Math.round((new Date(checkedOutAt).getTime() - startMs) / 1000));

    session.checked_out_at = checkedOutAt;
    session.duration_seconds = durationSeconds;
    session.status = 'COMPLETED';
    session.safety_note = safetyNote || 'Checked out safely';
    session.sync_status = isRemote ? 'SYNCED' : 'PENDING';

    if (this.db && this.isWasm) {
      try {
        const stmt = this.db.prepare(`
          UPDATE safegig_sessions 
          SET checked_out_at = ?, duration_seconds = ?, status = 'COMPLETED', safety_note = ?, sync_status = ?
          WHERE id = ?
        `);
        stmt.run([
        ...
        console.warn('SQLite completeSafeGigSession error:', e);
      }
    }
    ...
    if (!isRemote) {
      const payload = {
        ...
        safetyNote: session.safety_note
      };
      if (!offlineQueueService.isOnline()) {
        offlineQueueService.enqueueAction('CHECKOUT_SAFEGIG', payload);
      } else {
        syncService.broadcastSafeGigUpdate({ action: 'CHECKOUT', ...payload });
      }
    }
  ...
  public triggerSafeGigSos(
    ...
    const nowIso = new Date().toISOString();
    session.sos_activated = true;
    session.sos_timestamp = nowIso;
    session.status = 'SOS_TRIGGERED';
    if (locationData?.lat && locationData?.lng) {
      session.location_lat = locationData.lat;
      session.location_lng = locationData.lng;
      session.location_address = locationData.address || '';
      session.location_available = true;
    }
    ...
    if (this.db && this.isWasm) {
      try {
        const stmt = this.db.prepare(`
          UPDATE safegig_sessions 
          SET sos_activated = 1, sos_timestamp = ?, status = 'SOS_TRIGGERED', location_lat = ?, location_lng = ?, location_address = ?, location_available = ?
          WHERE id = ?
        `);
        stmt.run([
          nowIso,
          ...
          session.location_available ? 1 : 0,
          session.id
        ]);
        stmt.free();
      } catch (e) {
        console.warn('SQLite triggerSafeGigSos error:', e);
      }
    }
    ...
    if (!isRemote) {
      ...
      if (!offlineQueueService.isOnline()) {
        offlineQueueService.enqueueAction('TRIGGER_SAFEGIG_SOS', payload);
      } else {
        syncService.broadcastSafeGigUpdate({ action: 'SOS', ...payload });
      }
    }
  ...
  public applyRemoteSafeGigUpdate(data: any) {
    if (!data) return;
    const { action, session, sessionId } = data;
    if (action === 'START' && session) {
      if (!this.memorySafeGigSessions.some(s => s.id === session.id)) {
        this.memorySafeGigSessions.unshift({ ...session, sync_status: 'SYNCED' });
        this.notifyListeners();
      }
    } else if (action === 'CHECKOUT' && sessionId) {
      this.completeSafeGigSession(sessionId, data.safetyNote, true);
    } else if (action === 'SOS' && sessionId) {
      this.triggerSafeGigSos(sessionId, data.locationData, true);
    }
  }

  public updateUserEmergencyContact(userId: string, name?: string, phone?: string): User | null {
    const user = this.getUserById(userId);
    if (!user) return null;

    user.emergency_contact_name = name || '';
  ...
  public resetDatabase(): void {
    if (typeof localStorage !== 'undefined') {
      ...
      localStorage.removeItem(FALLBACK_SAFEGIG_KEY);
    }
    this.memoryReports = [];
    this.memoryOutcomes = [];
    this.memorySafeGigSessions = [];
    this.initFallbackData();

    if (this.db && this.isWasm) {
      try {
        this.db.run('DROP TABLE IF EXISTS jobs; DROP TABLE IF EXISTS users; DROP TABLE IF EXISTS reviews; DROP TABLE IF EXISTS notifications; DROP TABLE IF EXISTS reports; DROP TABLE IF EXISTS recommendation_outcomes; DROP TABLE IF EXISTS safegig_sessions;');
        this.initTablesAndSeed();
      } catch (e) {
