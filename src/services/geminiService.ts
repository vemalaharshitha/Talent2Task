    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 400 || res.status === 401 || res.status === 403 || res.status === 404) {
          quotaCooldownUntil = Date.now() + 300000;
          console.warn(`[GeminiService] Gemini API auth/model issue (${res.status}). Switching to instant offline local engine.`);
          throw new Error(`Gemini API auth error (${res.status})`);
        }
      ...
      if (err?.message?.includes('429') || err?.message?.includes('auth error') || Date.now() < quotaCooldownUntil) {
        throw err;
      }
      lastError = err;
    }
